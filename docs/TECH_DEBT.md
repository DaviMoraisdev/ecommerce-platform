...
- [5b DESCOBERTO - registrar] **O laco de reconexao (`agendarReconexao`) e o `consume` nao tem teste.** Mesma natureza da divida do `startOutboxRelay`/`stopOutboxRelay`: laco de timer com estado de modulo. O que esta provado e a topologia, a traducao de acao e o efeito no banco — nao o ciclo de vida da conexao.
- [5b DESCOBERTO - registrar] **A corrida do unique parcial de `pending_compensations` nao tem teste.** Duas transacoes concorrentes para o mesmo pedido cancelado: uma leva P2002, aborta junto com o inbox e vira requeue. O codigo trata (P2002 fora do inbox e relancado), mas nenhum teste exercita concorrencia real.
