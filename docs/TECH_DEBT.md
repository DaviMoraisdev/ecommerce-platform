...
## CART-SERVICE (Fase 4)
- **Corrida na validacao de estoque (addItem/updateQuantity):** o check-then-write (HGET/HEXISTS -> fetchProduct -> HINCRBY/HSET) nao e atomico; duas escritas concorrentes podem furar o limite de estoque no carrinho. Parte da divida de atomicidade/concorrencia — MULTI/Lua no hardening (Fase 7/10) + testes de concorrencia. Impacto soft: o carrinho nao e a reserva real (order-service e a autoridade). Levantado no review do PR #34.
- **Precisao monetaria:** subtotal/total usam number (float), com arredondamento para 2 casas como paliativo. Solucao correta: representar dinheiro em centavos (inteiro) de forma consistente entre servicos. Baixa.
