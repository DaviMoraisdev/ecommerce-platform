...
- [5b PAGO] **Primeiro consumer do order-service.** Fila `orders.payments`, DLX fanout + DLQ, `prefetch(1)`, e inbox transacional com `eventId @unique`.
  ...
  Prova: C1-C9 (traducao de resultado em acao) e C10-C15 (topologia e ack/nack) em unitario; G1-G8 em integracao contra Postgres real, cobrindo `@unique`, rollback e o unique parcial de `pending_compensations`.
...
- [5b DESCOBERTO - registrar] **Tres mecanismos de logging e de seguranca do handler nao tem teste que os pegue** (confirmado por sabotagem, nao suposto):
  (a) o `.catch` no `void tratarMensagem` do callback do broker — o C33 prova que `tratarMensagem` nao rejeita, nao que o callback observaria a rejeicao se ela ocorresse (sabotagem U4: nada cai);
  (b) a sanitizacao de `acao.reason` no ponto de saida para log — nenhum teste inspeciona a linha emitida (sabotagem U6-bis: nada cai);
  (c) o `try/catch` do ramo de payload grande tem teste (C33) mas NAO tem prova por sabotagem: nao houve como quebra-lo cirurgicamente sem erro de sintaxe (U5).
...
- [5b DEVOLVIDO - Bloco 8] **Retry sem contador nem fila de retardo.** Erro deterministico agora vai para a DLQ (`ehDeterministico`), o que remove o loop eterno por bug de programacao. Erro transitorio ainda requeueia indefinidamente — comportamento DESEJADO (a alternativa e descartar pagamento por indisponibilidade), mas sem teto e sem visibilidade de quantas tentativas ja houve. Retry queue com TTL e contador de `x-death` entra junto com o teste contra RabbitMQ real.
- [5b DEVOLVIDO - Fase 10] **`inbox_events` cresce sem politica de retencao.** Uma linha por captura processada, para sempre. REQUISITO que a rotina de purga precisa respeitar: a janela de retencao tem de ser MAIOR que o TTL maximo de mensagem no broker — purgar antes disso reabre a janela de duplicata para qualquer redentrega posterior a purga. Purga ingenua por idade transforma idempotencia em aposta.
