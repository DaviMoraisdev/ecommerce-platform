...
- **[6b PAGO] Job de reconciliação de tentativas presas.** Varredura periódica que pergunta ao provedor o desfecho de cada `AUTHORIZE` que ficou `PENDING` sem `providerRef` por mais que a janela, e aplica pelo MESMO caminho do fluxo normal (`aplicarDesfecho`), ou libera a tentativa quando o provedor prova que nada chegou. Componentes: `src/jobs/reconciliacao.ts` (decisão pura + varredura), `reconciliacao.repository.ts` (consulta de candidatos), `reconciliacao.runtime.ts` (ciclo de vida, poll default 60_000ms) e duas operações em `PaymentService` com compare-and-swap na MESMA transação do desfecho.
  DECISAO: a porta ganhou `buscarCobrancaPorTentativa(paymentId, attemptCount)`. `null` significa "nunca chegou" e e a UNICA evidencia que autoriza refazer a tentativa; falha tecnica LANCA, e a tentativa continua presa. Assimetria deliberada: aplicar desfecho errado mexe em dinheiro e e irreversivel do ponto de vista do cliente; nao aplicar mantem o estado atual, que ja e visivel.
  ...
  Prova (2026-08-28, commit 127f187): `npm run verify` 661/661 em 24 suites, mais build; `npm run verify:integration` 95/95 em 4 suites. Bateria de 6 sabotagens com a arvore ja commitada: as 6 derrubaram o alvo e `git status --porcelain` ficou vazio ao fim de cada uma. UMA delas passou VAZIA na primeira rodada — ver o item seguinte.
- **[6b DESCOBERTO — corrigido dentro do proprio bloco] O CAS de `liberarTentativaPresa` nao tinha teste que o pegasse.** O CASO J2 exercita o metodo sobre uma tentativa ainda `PENDING`, onde o `where` nunca precisa filtrar nada: remover `status: PENDING` e `providerRef: null` deixava as DUAS suites verdes. Sem a condicao, um `liberar` decidido sobre snapshot obsoleto sobrescreve um desfecho JA aplicado — transacao e `Payment` viram `FAILED` com o provedor tendo cobrado, e o `providerRef` some da trilha. Coberto pelo CASO J7 (integracao), que aplica o desfecho e so entao chama o `liberar`; com ele no lugar, a sabotagem passa a derrubar exatamente esse caso.
- **[6b DESCOBERTO — SUPOSICAO, nao verificada] A regra `null -> liberar` supoe consulta read-after-write consistente no provedor.** O fake responde da propria memoria, entao a suposicao nunca foi exercitada. Com um provedor real eventualmente consistente, um `null` FALSO libera para nova cobranca uma tentativa que ja cobrou — segunda cobranca, exatamente o que este bloco existe para evitar. E o unico ponto do job em que dinheiro pode ser duplicado, e o J7 nao protege contra ele (ali nenhum desfecho chegou a ser aplicado). **Gatilho:** troca do fake por provedor real (Bloco 9). Verificar a garantia documentada da consulta de cobranca; sem garantia, exigir confirmacao repetida ou janela maior antes de liberar.
- **[6b — pode esperar] `triagem` apenas incrementa contador e loga.** `recusa sem declineCode` e `cobranca cancelada no provedor` param o job de proposito e dependem de alguem ler um `console.warn`. **Gatilho:** observabilidade (logger estruturado e alerta), junto dos itens ja registrados na Fase 7.
- **[6b — pode esperar] O job nao tem lock entre instancias.** Com varias replicas, todas varrem o mesmo lote e consultam o provedor em duplicidade. A SEGURANCA nao depende disso: o compare-and-swap de cada operacao garante aplicacao unica (CASO J3) e bloqueia o liberar tardio (CASO J7); o custo e chamada duplicada ao provedor, nao dinheiro duplicado. Mesma natureza do claim atomico do relay ja registrado na Fase 7 — tratar junto, nao isolado.
