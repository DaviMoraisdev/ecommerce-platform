...
- [5b DESCOBERTO - registrar] **O CASO G11 nao prova disputa, so o desfecho.** Remover o CAS do `aplicarTransicao` (sabotagem V7) NAO o derruba — derruba o teste de concorrencia ja existente em `order.integration.test.ts`. As duas capturas disparadas juntas acabam serializadas, e a segunda le o pedido ja PAGO. Forcar entrelacamento deterministico exigiria controlar o agendamento das transacoes; sem isso o teste ficaria intermitente.
...
- [ESSENCIAL - Fase 7 Seguranca | SISTEMICO, nao introduzido pelo 5b] **Nenhum publicador de evento e autenticado.** O consumidor aceita a mensagem pela routing key e pela estrutura do JSON; nada prova que veio do payment-service. Quem puder publicar no exchange `payments` leva um pedido a PAGO ou cria uma compensacao. NAO e especifico deste PR: `order.created -> notification` esta na main com a mesma exposicao desde a Fase 3, e o broker usa credencial unica. A correcao e de infraestrutura — credencial por servico e ACL de publicacao, ou assinatura do evento — e vale para todos os pares de uma vez.
  PRE-CONDICAO DE ATIVACAO, nao divida comum: documentar nao protege. O consumidor de pagamentos NAO deve ser ligado em ambiente cujo broker seja acessivel a terceiros antes disso existir.
