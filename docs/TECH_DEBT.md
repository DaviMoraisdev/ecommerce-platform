...
- [5a DEVOLVIDO - ESSENCIAL, PR proprio no order-service] **O publisher do order-service tem os mesmos defeitos 4.2, 4.4 e 4.5 corrigidos aqui.** O publisher do payment e copia declarada do dele, e as correcoes ficaram so de um lado.
  ...
  Impacto: mensagem sem fila casada e marcada como enviada e perdida em silencio; canal pendurado deixa `connecting` preenchido e trava todo tick do relay ate reiniciar o processo; evento `error` de canal sem listener derruba o processo.
  NAO VERIFICADO: quais routing keys publicadas pelo order ficam sem binding. O unico consumidor no repo e o notification-service (`assertQueue`/`bindQueue` em `src/server.ts`). Se o binding cobre todas as keys, o defeito la e risco futuro; se nao cobre, ja ha perda silenciosa em curso. Verificar antes de priorizar.
