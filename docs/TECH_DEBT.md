...
- **[Fase 10] Processor de reconciliacao:** o estado DURAVEL da saga ja existe (tabela `idempotency_records` com orderId + status PROCESSING/COMPLETED/FAILED, criada ANTES das reservas; + `pending_compensations` para releases falhos). Falta o job que: (a) reprocessa `pending_compensations` com `resolvedAt` null; (b) reconcilia `idempotency_records` presos em PROCESSING (processo caiu no meio) liberando o orderId. Com retry/backoff idempotente, metricas e alerta.
