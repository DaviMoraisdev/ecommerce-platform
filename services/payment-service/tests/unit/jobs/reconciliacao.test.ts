...
describe('decidirReconciliacao', () => {
  it('CASO R1: ausencia de cobranca LIBERA a tentativa', () => {
    ...
    // nao pode causar segunda cobranca.
    expect(decidirReconciliacao(null)).toEqual({ tipo: 'liberar' });
  });

  it('CASO R2: cobranca capturada vira desfecho de sucesso', () => {
    const acao = decidirReconciliacao(snapshot({ capturedAmountCents: 12990 }));
    expect(acao).toEqual({
      tipo: 'aplicar',
  ...
  it('CASO R4: cobranca ainda em processamento NAO e tocada', () => {
    ...
    expect(decidirReconciliacao(snapshot({ state: 'PROCESSING', capturedAmountCents: 0 }))).toEqual(
      { tipo: 'aguardar' },
    );
  });
  ...
  it('CASO R5: cobranca CANCELADA vai para triagem, nao e aplicada', () => {
    ...
    const acao = decidirReconciliacao(snapshot({ state: 'CANCELED', capturedAmountCents: 0 }));
  ...
  it('CASO R6: recusa SEM codigo vai para triagem, nao vira recusa inventada', () => {
    ...
    const acao = decidirReconciliacao(snapshot({ state: 'DECLINED', capturedAmountCents: 0 }));
    expect(acao).toMatchObject({ tipo: 'triagem' });
  });
});
