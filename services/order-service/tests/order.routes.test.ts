...
describe('GET /orders/:id', () => {
  ...
  it('403 para outro usuario nao-admin', async () => {
...
describe('PATCH /orders/:id/status', () => {
  ...
  it('200 ADMIN muda status com changedBy = userId do JWT', async () => {
    mocked.updateOrderStatus.mockResolvedValue({ id: 'o1', status: 'PAGO' } as any);
    const res = await request(app)
      .patch('/orders/o1/status')
      .set('Authorization', token('ADMIN', 'admin1'))
      .send({ status: 'PAGO' });
    expect(res.status).toBe(200);
    expect(mocked.updateOrderStatus).toHaveBeenCalledWith('o1', 'PAGO', 'admin1');
  });
  it('404 pedido inexistente', async () => {
    mocked.updateOrderStatus.mockRejectedValue(new DomainError('PEDIDO_NAO_ENCONTRADO'));
    const res = await request(app)
      .patch('/orders/o1/status')
      .set('Authorization', token('ADMIN'))
      .send({ status: 'PAGO' });
  ...
  it('409 transicao invalida', async () => {
    mocked.updateOrderStatus.mockRejectedValue(new DomainError('TRANSICAO_INVALIDA'));
    const res = await request(app)
      .patch('/orders/o1/status')
      .set('Authorization', token('ADMIN'))
